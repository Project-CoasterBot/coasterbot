...
2 & 08.08.2026 & Borutta, Such, Etringer & Anpassungen nach Absprache \\
\bottomrule
\end{tabularx}
...
werden, der Getränkeuntersetzer selbstständig zu Gästen bringt, auf dem
Tisch platziert und nach Gebrauch wieder aufnimmt. Der entwickelte
Prototyp dient als Machbarkeitsnachweis für dieses Konzept.

\subsection{Projektziel}\label{projektziel}

Ziel dieses Projekts ist die Entwicklung eines Prototyps, nämlich des
...
\subsection{Projektumfang}\label{projektumfang}
...
\subsection{Projektabgrenzung}\label{projektabgrenzung}
...
\section{Systemübersicht}\label{systemuxfcbersicht}

\subsection{Systembeschreibung}\label{systembeschreibung}
...
Getränkeuntersetzer selbstständig zu Gästen zu transportieren, diese auf
dem Tisch zu platzieren und nach Gebrauch wieder aufzunehmen. Der Bot
bewegt sich dabei eigenständig innerhalb des ihm zur Verfügung stehenden
Arbeitsbereichs und erkennt Hindernisse sowie die Begrenzungen der
Tischoberfläche.
...
\subsection{Einsatzbereich}\label{einsatzbereich}
...
\subsection{Anwender}\label{anwender}
...
\subsection{Ausbaustufen}\label{ausbaustufen}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7730}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1773}}@{}}
\toprule\noalign{}
...
Arbeitsbereichs bestimmen können, solange er nicht manuell umgesetzt
wurde & Muss \\
...
einer definierten Positionsgenauigkeit erreichen. Der Zielpunkt darf
nach einem festen Schema vorgegeben sein. & Muss \\
...
erfolgreich abgesetzt wurde. & Muss \\
CST-006 & Der Coasterbot muss erkennen, ob ein Getränkeuntersetzer
erfolgreich aufgenommen wurde. & Muss \\
...
\subsection{Systemüberwachung}\label{systemuxfcberwachung}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0673}}
...
\subsection{Erweiterungsmöglichkeiten}\label{erweiterungsmuxf6glichkeiten}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7460}}
...
\subsection{Testbarkeit}\label{testbarkeit}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7156}}
...
\subsection{Simulationsfähigkeit}\label{simulationsfuxe4higkeit}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0560}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7440}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0511}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7664}}
...
NFA-REL-004 & Das System muss stabil stehen können und darf nicht von
selbst kippen können. & Muss \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1953}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Systemgrenzen}\label{systemgrenzen}
...
\subsection{Externe Akteure}\label{externe-akteure}
...
\subsection{Physische Schnittstellen}\label{physische-schnittstellen}
...
Der Coasterbot muss diese aufnehmen, transportieren, ablegen und wieder
aufnehmen können.

\subsubsection{Getränke}\label{getruxe4nke}

Getränke befinden sich auf der Tischoberfläche. Sie stellen Hindernisse
dar, die während der Navigation berücksichtigt werden müssen.
...
\subsection{Softwareschnittstellen}\label{softwareschnittstellen}
...
\subsection{Systemgrenzen zukünftiger
...
\subsection{Verifikationsverfahren}\label{verifikationsverfahren}

Zur Überprüfung der Anforderungen werden die folgenden
Verifikationsverfahren eingesetzt:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1523}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.8477}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Verfahren
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Abnahmekriterien}\label{abnahmekriterien}
...
Anforderung umgesetzt und überprüft wurde. Die Kennung ist ebenfalls in
den nachfolgenden Dokumenten zu verwendet.

\subsection{Abnahmeentscheidung}\label{abnahmeentscheidung}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.2659}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.7341}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Begriff
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Normen}\label{normen}
...
\subsection{Nationale Richtlinien}\label{nationale-richtlinien}
...
\subsection{Fachliteratur}\label{fachliteratur}
...
\subsection{Robotik}\label{robotik}
...
\subsection{Softwarearchitektur}\label{softwarearchitektur-1}
...
\subsection{Test und
Qualitätssicherung}\label{test-und-qualituxe4tssicherung}
...
\subsection{Wissenschaftliche Quellen des
Projekts}\label{wissenschaftliche-quellen-des-projekts}
...
Dokumente}\label{projektspezifische-dokumente}
