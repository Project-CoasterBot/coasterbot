...
Aktuierung \texttt{updateMotorControl()} übernimmt. Dies geschieht einmal mit
...
\item
  ...
  \texttt{measuredLeftSpeed()}/\texttt{measuredRightSpeed()} auch von
...
gemessen). Bei identischem Kommando auf beiden Seiten dürfte der Roboter
nicht abdriften.

Verifiziertes Ergebnis eines kopflosen Laufs:
...
exakt dem eingeprägten Gain-Fehler, wie erwartet; die Closed-Loop-Regelung
konvergiert innerhalb weniger Regelzyklen (deutlich unter \SI{1}{\second})
auf den Sollwert und hält die Kursdrift auf eine Größenordnung, die auch
ohne jeden Gain-Fehler allein durch das Restrauschen des simulierten Gyros
(Abschnitt~\ref{gyro-simulation}) zu erwarten wäre.
...
Regeldynamik ein, wo zuvor eine ideale Momentanantwort stand); alle
...
Die folgenden Punkte wurden vom Hardware-Team am 13.09.2026 zur
Schnittstelle \texttt{RobotHAL} zurückgemeldet. Drei Punkte (Radgranularität,
...
Auch diese Frage ist geklärt: Die reale Elektronik besitzt je
Fahrzeugseite nur \emph{einen} Encoder (und ohnehin nur eine Ansteuerung;
...
korrekt abbildet, siehe \enquote{Radpaarung} oben). \texttt{enum WheelId}
wurde deshalb von vier Werten (\texttt{WHEEL\_FL/FR/RL/RR}) auf zwei
reduziert (\texttt{WHEEL\_LEFT}/\texttt{WHEEL\_RIGHT}), und
\texttt{PoseEstimator} (Abschnitt~\ref{lokalisierung}) liest seither
direkt den Seitenwert statt einen Mittelwert aus zwei (zuvor fiktiven)
Einzelmessungen zu bilden. Da das PROTO in der Simulation weiterhin vier
unabhängige \texttt{PositionSensor} bereitstellt (ein Rasteraufbau des
CAD-Modells, kein Beleg für vier reale Messpunkte), mittelt
\texttt{WebotsHAL} Front- und Heckrad je Seite intern, um nach außen
exakt die Information eines einzelnen realen Encoders je Seite zu
liefern (Abschnitt~\ref{robothal}). Für eine künftige \texttt{ArduinoHAL}
entfällt dieser Mittelungsschritt, da sie ohnehin nur einen Encoder
je Seite liest.

\paragraph{Kantensensoren: \texttt{float} oder \texttt{bool}? (entschieden).}
Diese Frage wurde inzwischen entschieden: \texttt{RobotHAL} liefert die
vier Kantensensoren seit Version~3 dieses Dokuments als \texttt{bool}
...
unabhängig voneinander gegen dieselbe Konstante
\texttt{EDGE\_THRESHOLD = 500} auswerteten. Für die Algorithmik ist aber
tatsächlich nur die boolesche Aussage \enquote{Kante / kein Boden}
relevant, und die Kalibrierung eines IR-Kantensensors (Reflexionsgrad des
Untergrunds, Umgebungslicht, Sensorabstand) ist Hardwarewissen, das laut
...
Rohwert $>$ Schwelle $\Rightarrow$ \texttt{true}); eine künftige
\texttt{ArduinoHAL} kalibriert ihren eigenen Rohwert ebenso dort. Damit
entfällt zugleich die doppelte Ablage derselben Schwelle in
\texttt{robot\_logic.cpp} und \texttt{safety\_monitor.cpp}: Beide fragen
jetzt nur noch \texttt{hal.getEdgeFrontLeft() || \ldots} ab.

\emph{Nachtrag:} Die Kalibrierung wurde seither noch einen Schritt
...
Auf realer Hardware wäre dieser Schritt unverändert, die Integration ist
kontinuierlich: Anders als ein Encoder, der erst nach einer Mindestdrehung
einen Tick meldet, geht jede noch so kleine reale Drehung sofort in den
...
Gyros inzwischen (Abschnitt~\ref{gyro-simulation}), ein echter MEMS-Gyro
...
Rohwert mitteln, diesen Bias von jeder folgenden Messung abziehen), genau
das simuliert \texttt{WebotsHAL::calibrateGyro()} bereits. Für den
verbreiteten MPU-6050 (I\textsuperscript{2}C, 6-Achsen Gyro und
Beschleunigungssensor) existieren dafür fertige Arduino-Bibliotheken:
\texttt{Adafruit\_MPU6050} mit \texttt{Adafruit\_Sensor} (einfache API,
kalibrierte rad/s) oder die tiefer gehende \texttt{I2Cdevlib}-Bibliothek
(nutzt den chipinternen DMP für Sensorfusion bereits auf dem IMU). Eine
teurere Alternative wie der Bosch BNO055 erledigt die Sensorfusion
vollständig on-chip und liefert direkt eine fertige absolute Orientierung;
das könnte den Komplementärfilter in \texttt{PoseEstimator} überflüssig
machen, ist aber eine Architekturentscheidung für den Moment, in dem der
konkrete Chip feststeht. Ob die in Abschnitt~\ref{lokalisierung}
...
Beide damals skizzierten Umsetzungen (Open-Loop und Closed-Loop) wurden
...
\item
  ...
  die \texttt{RobotHAL}-Schnittstelle für alle aufrufenden Komponenten
...
des Treiber-ICs muss dafür ausreichen. Eine künftige \texttt{ArduinoHAL}
...
Aus dem Encoderwert je Seite folgt die gefahrene Strecke dieser Seite
...
Information wie der eine reale Encoder je Seite nachzubilden; siehe
Abschnitt~\ref{hal-offene-punkte}):
\[
...
\item
  ...
  $\approx \SI{0.012}{\degree\per\second}$, für die kurzen Testläufe
...
\texttt{setOccupied(Cell,bool)} & einzelne Zelle HART setzen; Rückgabe \texttt{true} bei Änderung (für inkrementelle Neuplanung)\\
...
\texttt{addObstacleRect()} trägt a priori bekannte Hindernisse hart und
sofort ein (z.\,B. die bekannte Bierflasche im Simulationstest). Das ist
gerechtfertigt, weil diese Eingabe nicht verrauscht ist. Ein per
Ultraschall oder Kantensensor während der Fahrt \emph{live} erkanntes
Hindernis ist dagegen eine einzelne Sensormessung und kann durch Rauschen
oder eine kurze Fehlmessung verursacht sein. Bis Version~5 dieses
...
Ausreißer reicht dagegen nicht. \texttt{main.cpp} verwendet
\texttt{reportOccupied()} inzwischen überall dort, wo bisher
\texttt{setOccupied()} für Live-Sensortreffer aufgerufen wurde
(\texttt{dstar\_scenario::markRect}, \texttt{sim\_scenario::markRect}).

Die schnelle Reflexbewegung bleibt davon unberührt: \texttt{DwaPlanner}
bewertet die Kollisionsfreiheit einer Trajektorie weiterhin \emph{jeden}
Zyklus direkt aus dem rohen Ultraschallwert als virtuellem Hindernispunkt
(Abschnitt~\ref{dwa-hindernisse}), ungeglättet. Geglättet wird
ausschließlich die \emph{deliberative} Entscheidung, ob D* Lite die
...
\texttt{MODE\_NAVIGATE\_DSTAR} liefern nach der Umstellung dieselben
...
\item
  ...
  Pose vollständig neu auf, was immer noch günstiger ist als eine
