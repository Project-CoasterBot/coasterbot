...
\item
  \textbf{Pflichtenheft}: die geplante technische Umsetzung, insbesondere
...
\item
  ...
  Bauteile, z.\,B.\ der Inertialsensor (Position 11: \enquote{ICQUANZX
...
\item
  ...
  \emph{keinen} Einzelrad-Befehl; \texttt{enum WheelId} hat entsprechend
  nur die zwei Werte \texttt{WHEEL\_LEFT}/\texttt{WHEEL\_RIGHT}.
...
\item
  ...
  \texttt{bool} zurück (\texttt{true} = Kante). Die Umrechnung des
  ...
  HAL-Implementierung und bleibt dort gekapselt (siehe
...
\item
  ...
  der Vertrag zwischen PROTO und Software.
\item
...
\item
  ...
  Encoder je Seite (\texttt{RobotHAL::WheelId} kennt entsprechend nur
  ...
  Rad); \texttt{getWheelAngle()} mittelt Front- und Heckrad je Seite, um
  nach außen dieselbe Information wie der eine reale Encoder zu liefern.
\item
  \textbf{Digitales Kantensignal:} Die \texttt{lookupTable} der vier
  Kantensensoren im PROTO liefert bereits ein digitales Signal (\num{0}
  oder \num{1}, kein analoger Rohwert; siehe
  Abschnitt~\ref{proto}). \texttt{digitalEdge()} liest diesen Pegel und
  bildet ihn über die private Konstante \texttt{EDGE\_ACTIVE\_HIGH} auf
  das semantische \texttt{bool} ab. Das ist die einzige Stelle im Code,
  an der bekannt sein muss, ob \enquote{Kante} am Sensor elektrisch HIGH
  oder LOW bedeutet; eine \texttt{ArduinoHAL}, die tatsächlich
  \texttt{digitalRead()} an einem Pin aufruft, bildet dort genauso ab und
  passt bei einem aktiv-low-Sensormodul nur diese eine Konstante an.
\item
  \textbf{Gyro-Simulation (GY-521/MPU-6050):} \texttt{getGyroZ()} prägt
  dem idealen, physikalisch simulierten Wert Bias und Rauschen des auf
  der Stückliste geführten Inertialsensors auf und kalibriert diesen
...
\item
  ...
  \texttt{setRightSpeed()} setzen nur noch das Kommando;
...
Die Stückliste (BOM) des Projekts führt unter Position 11 einen konkreten
...
Festlegung gilt als gegeben für die reale Hardware; die Simulation bildet
denselben Chip nach, statt einen idealen, fehlerfreien Sensor anzunehmen.
...
\item
  ...
  eine Annahme, kein Messergebnis.
\item
...
\texttt{calibrateGyro()} läuft im Konstruktor, \emph{bevor} irgendein
...
Diese Reihenfolge ist kein Detail, sondern notwendig: Ein früherer
Entwurf kalibrierte stattdessen laufend während der ersten
...
beobachtbar daran, dass der Roboter an einer falschen Pose hängen blieb
und keinen Testfall mehr bestand. Die Kalibrierung \emph{vor} dem ersten
möglichen Fahrbefehl behebt das; sie entspricht zugleich eher dem realen
Vorbild (Kalibrierung beim Einschalten, nicht während der Fahrt).

\paragraph{Folge für die Lokalisierung.} Weil nur \texttt{GYRO\_CAL\_SAMPLES}
$= 32$ Messwerte gemittelt werden, bleibt ein kleiner Restfehler im
...
werden, integriert \texttt{PoseEstimator} ihn über die Zeit zu einer
...
vernachlässigbar (siehe Abschnitt~\ref{lokalisierung-grenzen}); exakt
stabil war nur der vorherige, ideale (unsimulierte) Gyro.

\paragraph{Verifikation.} \texttt{MODE\_SIM\_TEST} wurde nach dieser
Änderung erneut zweifach kopflos ausgeführt: ST-SIM-001 bis ST-SIM-003
...
Kalibrierungsdauer von 32 Zyklen), die beiden Läufe weiterhin
...
Verhalten nicht zu verändern, machen die Simulation aber realitätsnäher.
...
Webots' Motormodell im Geschwindigkeitsmodus (\texttt{Motor::setVelocity()})
ist von sich aus \emph{ideal}: Ein Kommando wird exakt erreicht, ohne
Regelfehler. Ohne eine zusätzliche, absichtlich eingebaute
Störgröße hätten Open-Loop und Closed-Loop deshalb in der Simulation exakt
dasselbe Ergebnis geliefert und der Vergleich wäre bedeutungslos.
...
Getriebemotor-Toleranzen), kein Messwert eines realen Coasterbot-Exemplars.

\paragraph{Umsetzung in \texttt{WebotsHAL}.} \texttt{setLeftSpeed()}/
\texttt{setRightSpeed()} setzen seither nur noch das \emph{Kommando}
(\texttt{commandedLeft\_}/\texttt{commandedRight\_}); die eigentliche
Aktuierung übernimmt \texttt{updateMotorControl()}, einmal pro
\texttt{step()}, mit dem zuletzt gemessenen Encoderwert:
...
\item
  ...
  außen abrufbar (nur für diesen Vergleich; kein Teil der
