...
Schnittstelle \texttt{RobotHAL} zurückgemeldet. Zwei Punkte (Radgranularität,
Kantensensoren) wurden inzwischen entschieden und umgesetzt; die übrigen
sind weiterhin \textbf{offene Fragen} und hier dokumentiert, damit sie bei
der Portierung auf die Zielhardware (\texttt{ArduinoHAL}) nicht verloren
gehen.
...
setzt, wie das geschwindigkeitsgeregelte Webots-Motormodell, eine
gesteuerte (open-loop) Geschwindigkeitsvorgabe voraus. Ob sich damit auf
den realen Getriebemotoren tatsächlich ein definiertes rad/s einstellt,
ist offen; erste Erfahrungen zeigen ein unstetiges Ansprechverhalten der
Motoren.
...
Akkuspannung, Last und Bodenreibung ab. Zwei Umsetzungen sind denkbar:

\begin{itemize}
\tightlist
\item
  \textbf{Open-loop:} einmalig eine Kennlinie Tastgrad zu rad/s ausmessen
  und fest hinterlegen. Einfach, driftet aber mit sinkender Akkuspannung
  und mit Fertigungsstreuung zwischen den Motoren, wodurch der Roboter bei
  nominell gleichem Befehl links/rechts leicht kurvt statt geradeauszufahren.
\item
  \textbf{Closed-loop (empfohlen):} Falls nötig, müsste
  \texttt{ArduinoHAL::setLeftSpeed}/\texttt{setRightSpeed} intern eine
  Regelschleife führen, den einen Encoder je Seite lesen, die
  Ist-Geschwindigkeit schätzen und die Motoransteuerung per P- oder
  PI-Regler nachführen, sodass die \texttt{RobotHAL}-Schnittstelle für
  alle aufrufenden Komponenten unverändert bliebe. Die Regelung wäre
  vollständig in der Hardwaretreiberebene gekapselt.
\end{itemize}

Diese Regelschleife ist noch nicht umgesetzt, es gibt noch keine
\texttt{ArduinoHAL}. Zu beachten bei der Verdrahtung: Hängen beide Motoren
einer Seite an einem gemeinsamen H-Brücken-Kanal, addiert sich ihr
Stromverbrauch, das Rating des Treiber-ICs muss dafür ausreichen.
...
\item
  ...
  \texttt{getGyroZ()} sowie die offene Frage, ob
  \texttt{setLeftSpeed}/\texttt{setRightSpeed} auf realen Motoren eine
  Regelschleife benötigen; siehe die Rückmeldung des Hardware-Teams in
  Abschnitt~\ref{hal-offene-punkte} (Radgranularität und Kantensensoren
  sind aus derselben Rückmeldung bereits umgesetzt: \texttt{enum WheelId}
  auf \texttt{WHEEL\_LEFT}/\texttt{WHEEL\_RIGHT} reduziert, Kantensensoren
  als \texttt{bool}-Schnittstelle).
\end{itemize}
