...
bool WebotsHAL::simulateButtonPress(float duration) {
    const float now = getTime();
    if (_buttonState) {
        if (now - _buttonPressStartTime >= duration) {
            _buttonState = false;
            std::cout << "[HAL] simulated button released" << std::endl;
        }
    } else {
        if (now - _lastButtonReleaseTime >= 1.0f) { // 1 second cooldown
            _buttonState = true;
            _buttonPressStartTime = now;
            _lastButtonReleaseTime = now;
            std::cout << "[HAL] simulated button pressed" << std::endl;
        }
    }
    return _buttonState;
}

bool WebotsHAL::getButtonState() {
...
void WebotsHAL::led(LED_COLORS color) {
    if (ledColorInitialized_ && color == lastLedColor_) {
        return;
    }

    lastLedColor_ = color;
    ledColorInitialized_ = true;

    // In der Simulation wird die LED-Farbe ueber das PROTO-Field "ledColor"
    // gesetzt. Das PROTO-Field ist ein enum, das exakt zu dieser LED_COLORS-
    // Definition passt. Daher kann hier direkt der enum-Wert uebergeben
    // werden.
    switch (color) {
        case LED_COLORS::OFF:    std::cout << "[HAL] set LED OFF" << std::endl; break;
        case LED_COLORS::RED:    std::cout << "[HAL] set LED RED" << std::endl; break;
        case LED_COLORS::GREEN:  std::cout << "[HAL] set LED GREEN" << std::endl; break;
        case LED_COLORS::BLUE:   std::cout << "[HAL] set LED BLUE" << std::endl; break;
        case LED_COLORS::ORANGE: std::cout << "[HAL] set LED ORANGE" << std::endl; break;
    }
}